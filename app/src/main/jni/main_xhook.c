...
Java_com_example_jnitrace_MainActivity_startXhook(JNIEnv *env, jobject obj, jstring soname) {
    (void) env;
    (void) obj;

    (*env)->GetJavaVM(env, &global_jvm);
    //global_json = (*env)->NewGlobalRef(env, "44");
...
//    xhook_register("/data/.*\\.so$", "__system_property_read", new_system_property_read, (void **) (&old_system_property_read));
    ...
    char *soname_prefix = "/data/.*";
    char *soname_regex = malloc(strlen(soname_prefix) + strlen(soname) + 2);
    const char *soname_str = (*env)->GetStringUTFChars(env, soname, NULL);
    strcpy(soname_regex, soname_prefix);
    strcat(soname_regex, soname_str);
    strcat(soname_regex, ".so$");

    soname_regex = "/data/.*\\.so$";

    xhook_register(soname_regex, "strlen", new_strlen, (void **) (&old_strlen));
    xhook_refresh(1);
...
JNIEXPORT void JNICALL
Java_com_qiyi_biz_NativeHandler_test(JNIEnv *env, jobject obj) {
    (void) env;
    (void) obj;
